...
# set up test topography
dxdy = 10 # cell width and height of raster model grid

if flume1:
    dem = np.hstack(np.flipud(7*np.array([[10,10,8,10,10],[9,9,7,9,9],[8,8,6,8,8],
                              [7,7,5,7,7],[6,6,4,6,6],[5,5,3,5,5],[4,4,2,4,4],
                              [3,3,1,3,3],[2,2,0,2,2],[2,2,0,2,2],[2,2,0,2,2],
                              [2,2,0,2,2],[2,2,0,2,2],[2,2,0,2,2],[2,2,0,2,2],
                              [2,2,0,2,2],[2,2,0,2,2],[2,2,0,2,2]])).astype(float))
    mg = RasterModelGrid((18,5),dxdy)


if flume2:
    inar = np.array([[10.02,10.01,10,8,10,10.01,10.02],[9.02,9.01,9,7,9,9.01,9.02],[8.02,8.01,8,6,8,8.01,8.00],
                              [7.02,7.01,7,5,7,7.01,7.02],[6.02,6.01,6,4,6,6.01,6.02],[5.02,5.01,5,3,5,5.01,5.02],[4.02,4.01,4,2,4,4.01,4.02],
                              [3.02,3.01,3,1,3,3.01,3.02],[2,.02,.01,0,.01,.02,2],[2,.02,.01,0,.01,.02,2],[2,.02,.01,0,.01,.02,2],
                              [2,.02,.01,0,.01,.02,2],[2,.02,.01,0,.01,.02,2],[2,.02,.01,0,.01,.02,2],[2,.02,.01,0,.01,.02,2],
                              [2,.02,.01,0,.01,.02,2],[2,.02,.01,0,.01,.02,2],[2,.02,.01,0,.01,.02,2]])
    bb = np.reshape(inar[:,0],(len(inar[:,0]),1));
    inarray = np.concatenate((bb,inar,bb),axis =1)
    dem = np.hstack(np.flipud(7*inarray)).astype(float)
    bb = np.reshape(inar[:,0],(len(inar[:,0]),1));    
    cc = 4
...
if flume3: #31, c needs to be odd
    r = 30; #rows
    c =11; # columns
    sbr_r = 0.3; # ratio of length the slope break is placed, measured from the outlet
    cc = int(c/2)
...
if flume1:
    ...
    # mg.set_watershed_boundary_condition(dem) #finds lowest point in dem and sets it as an open node
if flume2:
    mg.set_watershed_boundary_condition_outlet_id(cc,dem)
if flume3:
    mg.set_watershed_boundary_condition_outlet_id(cc,dem)
...
if flume1:
    mg.at_node['mass__wasting_id'][np.array([57,62,67,72,77,82])] = 1
    
if flume2:
    mg.at_node['mass__wasting_id'][np.array([103,112,121,130,139,148])] = 1

if flume3:
    mg.at_node['mass__wasting_id'][lsn] = 1#np.array([178,195,212,229,246,263])] = 1    
# soil depth
depth = np.ones(nn)*2
...
for i in np.arange(0,len(example_MWRu.mw_ids)):
    ...
    for c in example_MWRu.df_evo_maps[i].keys():
        ...
        LLT.surf_plot(mg,surface='df_topo',color_type = "grey", min_s = 1, max_s = 1, constant_scale = True, dv = 1.5,
...
for i, topo in enumerate(example_MWRu.te_r[1]):
    if i%pi == 0:
        ...
        plt.plot(x_,y_,'r-', alpha = 0.5, linewidth = 1, label = 'post mw profile')
        plt.ylim([-3,max(y)*.4]); plt.ylabel(' elevation ')
        plt.xlim([0, max(x_)*.5])
        plt.legend()
        plt.grid(alpha = 0.5)
        # plt.gca().set_aspect('equal')
