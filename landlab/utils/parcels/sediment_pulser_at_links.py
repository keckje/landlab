...
class SedimentPulserAtLinks(SedimentPulserBase):
    ...
    def __call__(self, time, links=[0], n_parcels_at_link=[100], d50 = None, 
        ...
        # if parameter is not specified with __Call__ method, use class value across network
        if d50 is None: 
        ...
        if (np.array([d50,std_dev,rho_sediment, parcel_volume, abrasion_rate]) < 0).any():# check for negative inputs         
           raise AssertionError('''grain size median grain size standard deviation 
                                and parcel volume can not be less than zero''')
        ...
        # assume d50 can be used to approximate mean
    ...
    def _sediment_pulse_stochastic(self,
        ...
        abrasion_rate = np.full_like(element_id, self._abrasion_rate, dtype=float)
        density = np.full_like(element_id, self._rho_sediment, dtype=float)
