# -*- coding: utf-8 -*-
"""
This script sets up a raster model grid and a network model grid. The network
model grid is set up using landlabb.utils.parcels BedParcelInitializer

Given a precipitation time series, it models shallow landslide location on 
hillslopes using the LandslideProbability component and flow depth in channels 
using a simply hydraulic geometry relation between depth and contributing area.

It then models landslide runout path on the raster model grid and converts the 
location to a network model grid location using MassWastingRouter

It then adds the landslide deposits as parcels to the network model grid using
the landlabb.utils.parcels SedimentPulser class. 
...
from landlab.components import StreamPowerEroder, PrecipitationDistribution, FlowAccumulator, DepressionFinderAndRouter
...
os.chdir('C:/Users/keckj/Documents/GitHub/code/landlab')
import MWR_v3 as MWR
#%% RASTER MODEL GRID FOR LANDSLIDE PROBABILITY and MASS WASTING ROUTER SECTION

#load raster model grid that contains all fields needed for LandslideProbability component, add fields needed for MassWastingRouter component

th = 0.99#find values that have probability of failure greater than th

#import raster grid created by running LandslideProbability component 'LandslideGrid.py'
ddir = 'D:/UW_PhD/PreeventsProject/Paper_2_MWR/Landlab_Development/network_transporter/'
os.chdir(ddir)
mg = read_netcdf('NST_setup_landslides_v5.nc')

#reload topographic elevation using dem from esri_ascii command to avoid landlab error
(mgt, z) = read_esri_ascii('demv3.TXT', name='topographic__elevation')
sz = z.shape 
...
#run DepressionFinderAndRouter to fill holes
...
x_of_node = tuple(np.array(x_of_node)+150)
...
grid.at_link["reach_length"] = np.array([ 100.0,  103.1, 111.8, 141.4, 111.8,  
                                         111.8,  180.3]) # m
...
#%% INSTANTIATE NETWORKSEDIMENTTRANSPORTER, SEDIMENT PULSER AND MASSWASTINGROUTER CLASSES
...
# initial landslide probability

initial_probability = np.ones(sz)*0
mg.at_node['MW__probability'] = initial_probability   
clumps = MWR.MassWastingRouter(mg,grid,probability_threshold = th,Link_Length = 
...
for c, ts in enumerate(range(timesteps)):
    print(c)
    
    # daily average hourly precipitation
    P = Pt[c] #daily precipitation [mm]
    
    # update flow depth
    grid.at_link["flow_depth"] = fdepth[:, ts]
    
    # run landslideprobability
    ls_prob = LandslideProbability(mg, number_of_iterations=10,
                     groundwater__recharge_distribution='uniform',
                     groundwater__recharge_min_value=P-.0001,
                     groundwater__recharge_max_value=P+.0001)
  
    ls_prob.calculate_landslide_probability()
    
    # update MW__probability field
    mg.at_node['MW__probability'] = mg.at_node['landslide__probability_of_failure']

    a_df = pd.DataFrame(mg.at_node['landslide__probability_of_failure']) 
    a_th = a_df[a_df>th]
    # nodes = mg.nodes.reshape(mg.shape[0]*mg.shape[1],1)
    a_thg = a_th.values
    
    # update threshold_probability field (all points with annual probability greater than threshold)
    # print(a.max())
  
    # delete field if already exists
    try:
        mg.delete_field('node','threshold_probability')
        mg.at_node['threshold_probability'] = a_thg    
        mg.set_nodata_nodes_to_closed(mg.at_node['threshold_probability'], -9999)
    except:
        mg.at_node['threshold_probability'] = a_thg    
        mg.set_nodata_nodes_to_closed(mg.at_node['threshold_probability'], -9999)            
       
    # run Mass Wasting Router one step (instantly delivers all landslides to channel)
    clumps.run_one_step(dt)
    
    # add a pulse of material to links
    make_pulse(time = nst._time, d50 = P_d50, 
               max_parcel_volume = P_parcel_volume, 
               parcelDF = clumps.parcelDF) # run __call__ and parcels.add_item
