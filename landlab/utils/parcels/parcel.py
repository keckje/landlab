...
class SedimentPulser:
    ...
    Presently, this utility has two methods for adding sediment parcels to the 
    DataRecord:
    
    (1) add parcels by specifying a list of links, list of median grain size and 
    a list of the number of parcels added to each link. The parcels are then 
    randomly placed into each link. All parcels placed in a link have the same
    grain size.
    
    
    (2) add parcels by specifying the link, median grain size and location in 
    the link in a pandas dataframe.

    If a DataRecord is not specified when SedimentPulser is instantiated, 
    the pulse of sediment establishes the DataRecord that all future pulse are
    added to.
    ...
    def __call__(self, time, d50=[0.03], n_parcels_at_link=[100], links=[0], max_parcel_volume = 0.05, parcelDF = None):
        ...
        if parcelDF is None:
            ...
            if parcelDF.empty == True:
                return self._parcels
            
            variables, items = _pulse_characteristics_user_defined(
                parcelDF,
                max_parcel_volume,
                time,
                point_pulse = True)
...
def _pulse_characteristics_user_defined(parcelDF,parcel_vol,time,
    ...
    for index, row in parcelDF.iterrows():
        p_np.append(int(row['vol [m^3]']/parcel_vol)) #number of parcels in pulse = volume pulse/volume 1 parcel
    ...
    p_parcel_D  = .03  # grain size in parcel : Change to read parcelDF
