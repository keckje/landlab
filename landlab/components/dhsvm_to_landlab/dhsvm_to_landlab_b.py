...
class DHSVMtoLandlab(GridTTools):
    ...
    def __init__(self,
        ...
        if (nmgrid != None) and (grid != None):
            ...
            # GridTTools.__init__(nmgrid)
            raise ValueError("a raster model grid is required to map the dhsvm" \
                         " network model grid to the landlab network model grid")
        else:
            raise ValueError("a network model grid or raster model grid required" \
                             " to run DHSVMtoLandlab")


        # check for required network model grid inputs
        if nmgrid != None:
            if nmgrid_d != None:
        ...
        if (nmgrid != None) and (grid != None):
            ...
            Q_l = self._streamflowonly_ag[rep_reach]
            self.PDS = self.partial_duration_series(Q_l, self.sed_ri)
            # self.AMS = self.partial_duration_series(Q_l, 1)
            
            self.P_Fx, self.P_x1, self.P_T, self.P_Ty, self.P_Q_ri  = self.fit_probability_distribution(AMS=self.PDS['value'],
    ...
    def run_one_step(self, ts = None):
        ...
        if self._time_idx < len(self.storm_dates)+1:
            ...
            if ts is None:
                ...
                # if depth to water tablesupdate depth to water table if this is true, updating water table is slow                    
                if self.rpm >0:
    ...
    def fit_probability_distribution(self, AMS, min_ri = 1, dist = 'LN', 
        ...
        # convert return period per minimum event to annual return period
        Ty = T/(1/min_ri)
