...
class DHSVMtoNMG(GridTTools):
    ...
    def __init__(self,
        ...
        if precip is None:
            ...
            # determine precip statistics
            self.PDS = partial_duration_series(self.precip)
            
            P_Fx, P_x1, P_T, P_Q_ri  = fit_probability_distribution(AMS=self.PDS['value'])
        ...
        for c, Link in enumerate(self.LinkMapper.keys()):
            ...
            PDS_Q_l = partial_duration_series(Q_l)
            Q_l_Fx, Q_l_x1, Q_l_T, Q_l_Q_ri  = fit_probability_distribution(AMS=PDS_Q_l['value'])
    ...
    def __call__(self, ts=None):
        ...
        if self._method == 'time_series':
           ...
           q_i = np.random.uniform(LL,P_Fx.max(),1)# pick a random quantile between the quantile hydrology used for bedload
           
           # get the P rate
           P_i = intp(P_Fx,P_x1,q_i)
           
           # get equivalent Q rate for each link
...
###
    ...
    def RI_flows(self):
        ...
        for Link in self.LinkMapper.keys():
            ...
            PDS = partial_duration_series(Qts)
    ...
    # def run_one_step(ts = None):
    #     if ts is None:


def annual_maximum_series(time_series, plotting_position = 'Cunnane'):
    '''
...
def partial_duration_series(time_series, plotting_position = 'Cunnane',sep=30):
    '''
...
def fit_probability_distribution(AMS, dist = 'LP3', 
                   RI=[1.5,2,5,25,50,100], plotting_position = 'Cunnane',
                   print_figs = False):
    '''
    ...
    AMS : pd series, index is pd.datetime
        annual maximum series (or partial duration series if partial
                               ...
                               series)
    dist : string
        cane be 'LN', 'GUMB', 'GEV', 'P3', 'LP3'.  default is 'LP3'
        
        type of distribution fit to AMS. Can be either: lognormal, gumbel, 
        GEV, Pearson type III or log-Pearson type III.
    ...
    #   parameters, maidment 1992, p18.14 
        ...
        x1 = np.linspace(1,2*max(AMS),10000)  #solve pdf at fine resolution
    ...
    if dist =='GUMB':
        ...
        x1 = np.linspace(1,2*max(AMS),10000)
        
        x2 = np.sort(AMS, axis=0)
        X = [x1,x2]
    ...
    if dist == 'P3':    
        ...
        x1 = np.linspace(np.ceil(xi),1.5*max(AMS),10000) 
