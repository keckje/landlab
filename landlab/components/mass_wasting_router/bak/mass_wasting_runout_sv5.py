...
class MassWastingRunout(Component):
    ...
    def _scour_entrain_deposit(self):
        ...
        for i,inn in enumerate(innL):
            ...
            # while there are receiving cells 
            while len(arn)>0 and c < self.itL:
                ...
                # determine the incoming flow thickness, erosion, deposition, out going flow thickness 
                for n in np.unique(arn):
                    ...
                    if self._grid.at_node['drainage_area'][n] < 2000:
                        slpc = 0.7
                    else:
                        slpc = self.df_dict['critical slope']     
                        
                    # slpc = np.min([441.24*self._grid.at_node['drainage_area'][n]**(-0.596), 0.7])
                    
                    Lnum = np.max([(1-(slpn/slpc)**2),0])
                    
                    dpv = vin*Lnum # deposition volume
                    
                    # determine erosion depth
                    
                    # debris flow depth over cell at begin of iteration    
                    df_depth = vin/(self._grid.dx*self._grid.dx) #df depth
                    
                    # determine erosion depth (function of slope)                   
                    ...
                    if vin <=SV:
                        dpv = vin # all volume that enter cell is deposited 
                        vo = 0 # debris stops, so volume out is 0
        
                        # determine change in cell height
                        deta = (dpv)/(self._grid.dx*self._grid.dy) # (deposition)/cell area
        
                    else:
                        # determine change in cell height
                        deta = (dpv-ev)/(self._grid.dx*self._grid.dy) # (deposition-erosion)/cell area                        

                        # if deta exceeds regolith thickness, deta equals regolith thickness (fresh bedrock is not eroded...this is redundant)
                        if self._grid.at_node['soil__thickness'][n]+deta <0:
                            deta = - self._grid.at_node['soil__thickness'][n]  
                        
                        vo = vin-dpv+ev # vol out = vol in - vol deposited + vol eroded
                        

        
                    # debris flow depth at end of iteration, use to determine movement of next iteration
                    df_depth = vo/(self._grid.dx*self._grid.dy) # (flow thickness = vol out/cell area)
                    
                    # update raster model grid regolith thickness, bare dem and dem
                    ...
                    self._grid.at_node['topographic__elevation'][n] = self._grid.at_node['topographic__elevation_bare'][n]+df_depth
                ...
                if self.save:
                    ...
                    # save precipitions
                    self.df_th_maps[i][c] = self._grid.at_node['topographic__elevation']-self._grid.at_node['topographic__elevation_bare']
