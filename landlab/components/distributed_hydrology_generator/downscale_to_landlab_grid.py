...
class downscale_to_landlab_grid():
    ...
    def __init__(self, H_grid,
        ...
        if H_lambda and landlab_lambda and landlab_soild: 
            print('interpolating maps to landlab grid using the topographic wetness index')

            # dhsvm wetness index
            g2, wi_d = read_esri_ascii(H_lambda, name='lambda')
            self.H_lambda = self._grid_to_da(g2,'lambda')
    ...
    def _appended_maps_to_dataset(self):
        ...
        DHSVM_dwt = xr.Dataset(data_vars = {self.var_name: (('time', 'x', 'y'), separate_maps)})
    ...
    def _topmodel_interpolate_H_to_landlab(self):
        ...
        for i,m in enumerate(self.dtw_d_l.time):
            dtw_d_l_single = np.squeeze(self.dtw_d_l.isel(time=[i])[self.var_name])
        ...
        landlab_dtw = xr.Dataset(data_vars = {self.var_name: (('time', 'x', 'y'), landlab_dtw_)})
        #assign coordinates and plot again 
