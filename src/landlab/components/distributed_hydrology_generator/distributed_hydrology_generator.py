"""
TODO: 
    add a check for nan values in flow or soil water data, add error message
...
class DistributedHydrologyGenerator(Component):
    ...
    def __init__(self,
        ...
        # plotting position for empirical estimate of non-exceedance probability
    ...
    def channel_hydraulics(self, Link):
        ...
        U,Uo,nt,no,T,Teff,Teffr,deff,Teff_s1,Teff_s2,daily_erosion_depth = flow_resistance_RR(q,d,S,D65,D84)
    ...
    def _min_return_interval(self, dates):
        ...
        if self.plotting_position == 'Cunnane':
            PP = (ranks-.4)/(Yrs+.2) #compute plotting position 
        elif self.plotting_position == 'Weibull':
            PP = ranks/(Yrs+1)
        elif self.plotting_position == 'Blom':
    ...
    def partial_duration_series(self, time_series, RI_cutoff = 1, sep=30):
        """
    ...
    def fit_probability_distribution_new(self, MS, min_ri = 1, dist = 'LN', 
                       visually_check_fit = False):
        """
        
        Fits a log-normal or log-Pearson III distribution to an annual maximum series 
        or partial duration series using existing numpy and scipy functions.
           
        IF the distribution is fit to an annual maximum series, the resultant cdf 
        describes the annual probability of non-exceedance as a function of event magnitude.
        
        If the distribution is fit to a partial duration series that lists magnitudes
        less than a return interval of 1 year, then the resultant cdf describes the
        event-probability of non-exceedance as a function of event magnitude. 
     
        e.g., if the distribution is fit to a partial duration series of maximum flow
        rates and includes events as small as the 0.25 year event,then the fit cdf 
        gives the likelihood of not exceeding a given flow magnitude during any 0.25 year 
        and larger flow event. 
        ...
        Parameters
        ----------
        MS : pd series, index is pd.datetime
            annual maximum or partial duration series
        min_ri: float
        ...
        dist : string
            ...
            log-Pearson type III. Can be 'LN' or 'LP3'. Default is 'LN'
        visually_check_fit : boolean
            plot fit distribution relative to a normalized histogram of the 
    
        Returns
        -------
        Fx : np.array
            quantile or non-exceedance probability (domain of cdf, 0 to 1)
        x : np.array
            range of values
        T : np.array
        ...
        if dist == 'LP3':
            x, Fx, fx = logPearsonIII(MS)   
            
    
        if visually_check_fit:
            # visual check that distribution fits: 
            n = MS.shape[0] # number ov values in maximum series
            ranks = np.array(range(1,n+1)) # rank
            x2 = np.sort(MS, axis=0) # sorted maximum series
            if self.plotting_position == 'Cunnane':
            ...
            ax[0].hist(MS,bins='auto', density=True, alpha=0.6, color='b', label='raw data')  # arguments are passed to np.histogram
        ...
        # event return interval from non-exceedance probability of fit distribution 
        T = 1/(1-Fx) 
        ...
        if min_ri < 1:
            ...
            Ty = T
        # quantile of each value in range_of_values, range_of_values, return period, return period in years, flow value for a specific return period, likelihood of each value in reange of values, exceedance probability, sorted (large to small) annual maximum values 
        return Fx, x, T, Ty
    

def logPearsonIII(sample):
    """
    Fit a log-Pearson III distribution to a sample of hydrologic data such as
    an annual maximum or partial duration seres of flow.

    Parameters
    ----------
    sample : numpy array
        the sample of hydrologic data

    Returns
    -------
    x : numpy array
        a range of values, uniformly spaced between 50% of the minimum value
        and 150% of the maximum value in the sample.
    Fx : numpy array
        the cdf or non-exceedance probability value for each value x 
    fx : numpy array
        the pdf or liklihood value for each value x

    """
    # transform values 
    logged_sample = np.log(sample)
    # get Pearson III shape parameters of log transformed data
    skew, loc, scale = pearson3.fit(logged_sample)
    # create a range of values based on the min and max values in the sample
    x = np.linspace(min(sample) * 0.5, max(sample) * 1.5, 1000)
    # then transform
    x_logged = np.log(x)
    # get the log-Pearson III pdf liklihood values at each x_logged value, using shape parameters from
    # logged_sample and transform back to linear scale by dividing by x
    # this approach based on https://gemini.google.com/app/fb025872baff331c
    fx = pearson3.pdf(x_logged, skew, loc, scale)/x
    # integrate the pdf to get the non-exceedance probability (cdf) values
    Fx = sc.integrate.cumulative_trapezoid(fx, x, initial=0)
    return x, Fx, fx
...
def lognorm(sample):
    """
    Fit a log-normal distribution to a sample of hydrologic data such as
    an annual maximum or partial duration seres of saturated thickness.
    ...
    """
    # get log normal distribution parameters from sample
    scale = np.exp(np.nanmean(sample)) #  use exp of mean, see https://github.com/scipy/scipy/issues/12844
    s = sample.std()
    x = np.arange(sample.min()*0.5, sample.max()*1.5,.0001)
    # probability distribution value (liklihood) for each 
    fx = sc.stats.lognorm.pdf(x=x,s=s,scale = np.log(scale))
    # integrate the pdf to get the non-exceedance probability (cdf) values
    Fx = sc.integrate.cumulative_trapezoid(fx, x, initial=0)
    return x, Fx, fx
