...
class MWR_Calibrator():
    ...
    TODO:   make slpc_exponent and slpc_coefficient calibrator options
        
            See other Bayesian calibration apporaches that use more than two variables
    ...
    def __init__(self,
        ...
        self.grid = MassWastingRunout.grid
        ...
        if np.any(pvals <0):
            raise ValueError("check MCMC_adjusted_parameters for negative values")
        ...
        elif "landslide_thickness_uniform" in MCMC_adjusted_parameters.keys() and "landslide_thickness_multiplier" in MCMC_adjusted_parameters.keys():
                msg = "candidate values include both landslide_thickness_uniform and landslide_thickness_multiplier parameters, pick one"
                raise ValueError(msg)  
        else:
        ...
        self.initial_soil_depth = self.grid.at_node['soil__thickness'].copy()
        if self.grid.has_field("particle__diameter", at="node"):
            self.initial_particle_diameter = self.grid.at_node["particle__diameter"].copy()
        self.plot_calibration_metrics = plot_calibration_metrics
        self.DoD_m_dict ={}
        self.trial_Qs_profiles = {}
        self._maker(seed)
        self.jstracking = [] # for tracking jumps
        
        if (alpha_min < 0 or alpha_min > 1)  or  alpha_max < alpha_min or alpha_max > 1 or phi_minus < 0 or phi_plus < phi_minus:
            msg = "check adaptable jump variance parameters alpha and phi"
            raise ValueError(msg)  
        else:
    ...
    def __call__(self, max_number_of_runs = 50):
        ...
        if self.calibration_method == "extent_and_sediment":
            ...
            self.TMV = (-1*(self.grid.at_node['DoD_o'][self.grid.at_node['DoD_o']<0]).sum())*self.grid.dx*self.grid.dy
            # mean total flow
            self.profile_metric_mean = self.mbLdf_o[self.profile_metric].mean()
        else:
            self.TMV = np.nan
            self.profile_metric_mean = np.nan
    ...
    def _simulation(self,candidate_values):
        ...
        self.grid.at_node['disturbance_map'] = np.full(self.grid.number_of_nodes, False)
        if self.grid.has_field("particle__diameter", at="node"):
            self.grid.at_node["particle__diameter"] = self.initial_particle_diameter.copy()
        ...
        diff = self.grid.at_node['topographic__elevation'] - self.grid.at_node['topographic__initial_elevation']
        self.grid.at_node['DoD_m'] = diff
        
        # save the DoD
        self.DoD_m_dict[self.it] = self.grid.at_node['DoD_m']
        
        if self.plot_calibration_metrics == True:
            it = self.it
            plt.figure('iteration'+str(self.it))
            imshow_grid(self.grid,"DoD_m",cmap = 'RdBu_r')
            plt.title(f"iteraion {it}: "+", ".join([f"{k} = {v:.3f} " for k, v in candidate_values.items()]))
            plt.clim(-1,1)
            plt.show()
    ...
    def _update_topographic_slope(self):
        ...
        fd = FlowDirectorMFD(self.grid, surface="topographic__elevation", diagonals=True,
    ...
    def _downslope_A(self, dem, demd, el):
        ...
        A_d = dAs*self.grid.dx*self.grid.dx # multiply by cell area to get volume  
        return A_d
    ...
    def _upslope_E(self, dem, demd, el):
        ...
        E_u = dEs*self.grid.dx*self.grid.dx   
        return E_u
    ...
    def _downslope_A_and_E(self, dem, demd, el):
        ...
        EA_d = dAEsd*self.grid.dx*self.grid.dx 
        return EA_d
    ...
    def _Qs(self, dem, demd, el):
        ...
        Qs = dAEsu*self.grid.dx*self.grid.dx
        return Qs
    ...
    def _omegaT(self, metric = 'entire_runout_extent'):
        ...
        Heiser et al. (2017)
        """
        n_a = self.grid.nodes.reshape(self.grid.shape[0]*self.grid.shape[1]) # all nodes
        if metric == 'entire_runout_extent':
            n_o =  n_a[np.abs(self.grid.at_node['DoD_o']) > 0] # get both aggradation and erosion nodes
            n_m = n_a[np.abs(self.grid.at_node['DoD_m']) > 0]
        elif metric == 'deposition_extent_only':
            n_o =  n_a[self.grid.at_node['DoD_o'] > 0] # get aggradation nodes
            n_m = n_a[self.grid.at_node['DoD_m'] > 0]
        elif metric == 'scour_extent_only':
            n_o =  n_a[self.grid.at_node['DoD_o'] < 0] # get erosion nodes
            n_m = n_a[self.grid.at_node['DoD_m'] < 0]
        self.a_o = n_o
        self.a_m = n_m
    ...
    def _SE_DoD(self, metric = 'entire_runout_extent'):
        ...
        """
        c1 = 1; c2 = 1
        n_a = self.grid.nodes.reshape(self.grid.shape[0]*self.grid.shape[1])
        na = self.grid.dx*self.grid.dy
        if metric == 'entire_runout_extent':
            n_o =  n_a[np.abs(self.grid.at_node['DoD_o']) > 0] 
            n_m = n_a[np.abs(self.grid.at_node['DoD_m']) > 0]
        elif metric == 'deposition_extent_only':
            n_o =  n_a[self.grid.at_node['DoD_o'] > 0]
            n_m = n_a[self.grid.at_node['DoD_m'] > 0]
        elif metric == 'scour_extent_only':
            n_o =  n_a[self.grid.at_node['DoD_o'] < 0]
            n_m = n_a[self.grid.at_node['DoD_m'] < 0]
        ...
        A_x = len(n_x)*self.grid.dx*self.grid.dy
        A_u = len(n_u)*self.grid.dx*self.grid.dy
        A_o = len(n_o)*self.grid.dx*self.grid.dy
        CA = self.grid.dx*self.grid.dy
        observed_ = self.grid.at_node['DoD_o']

        modeled_ = self.grid.at_node['DoD_m']       
    ...
    def _MSE_Qs(self):
        ...
        if self.plot_calibration_metrics == True:
            ...
            plt.ylabel('Qs, m^3')
            plt.xlabel('distance from landslide crown')
            plt.title(nm)
            plt.legend()
            plt.show()  
    ...
    def _get_a_candidate_value(self, selected_value, key):
        ...
        pass_ = False
        # candidate value can't be outside of the max and min values
    ...
    def _check_E_lessthan_lambda_times_qsc(self, candidate_values, jump_sizes, selected_values):
        ...
        equivalent_E = erosion_rate(self.MWR.k, self.tau, self.MWR.f, self.grid.dx)*self.grid.dx # rate times cell width to get depth
        _lambda = self._lambda_low_slope 
        if self.MWR.slpc.min()>=0.02: # when slpc is high (>0.02), model is unstable when ~E>(10*qsc)
        ...
        if equivalent_E>self.MWR.qsc*_lambda: # candidate values have already been transfered to MWR at this point
            ...
            if self.params.get('k'):
                ...
                equivalent_E_min = erosion_rate(max(self.params['k'][2]-4*self.jump_size*(self.params['k'][1]-self.params['k'][0]),self.params['k'][0]),self.tau,self.MWR.f,self.grid.dx)*self.grid.dx # if erosion from k-four standard deviations (jumpsize)) greater than qsc, then the lower range needs to be decreased
                ...
                if equivalent_E_min>self.MWR.qsc*_lambda:
                    ...
                    while _pass is False:  # repeatedly jump from the selected parameters until candidate_equivalent_E < qsc*_lambda
                        candidate_values['k'], jump_sizes['k'] = self._get_a_candidate_value(selected_values['k'], 'k')
                        candidate_equivalent_E = erosion_rate(candidate_values['k'],self.tau,self.MWR.f,self.grid.dx)*self.grid.dx
            ...
            # if k is not a calibration parameter (k is fixed), then adjust qsc to meet constraint
                ...
                if equivalent_E > self.params['qsc'][2]+min(4*self.jump_size*(self.params['qsc'][1]-self.params['qsc'][0]),self.params['qsc'][1])*_lambda: # if erosion is less than lambda*(qsc+four standard deviations (jumpsize)), then the upper range needs to be increased
                    ...
                    _i_ = 0
                    while _pass is False: # repeatedly jump from the selected parameters until candidate_equivalent_E < qsc*_lambda
                        candidate_values['qsc'], jump_sizes['qsc'] = self._get_a_candidate_value(selected_values['qsc'], 'qsc')
    ...
    def _adjust_jump_variance(self, acceptance_ratio):
        """following LeCoz et al., 2014 (see the MCMC tutorials), adjust jump 
        variance based on acceptance ratio"""

        if acceptance_ratio < self.alpha_min: # if acceptance ratio too low, reduce jump variance to increase acceptance rate
            factor = self.phi_minus**0.5 # other approaches multiply by the factor without the square root, doing it this way reduces the factor
        elif acceptance_ratio > self.alpha_max: # if to high, increase jump variance
    ...
    def _update_MWR_parameter_values(self, candidate_values):
        ...
        for key in self.params:
            if key == 'qsc':
                ...
                self.MWR._grid.at_node['soil__thickness'][self.MWR._grid.at_node['mass__wasting_id'] < 1] = candidate_values[key]
            elif key == "landslide_thickness_multiplier":
                self.MWR._grid.at_node['soil__thickness'][self.MWR._grid.at_node['mass__wasting_id'] >= 1] = ( 
                self.MWR._grid.at_node['soil__thickness'][self.MWR._grid.at_node['mass__wasting_id'] >= 1]*candidate_values[key]
                )
            elif key == "soil_thickness_multiplier":
                self.MWR._grid.at_node['soil__thickness'][self.MWR._grid.at_node['mass__wasting_id'] < 1] = (
                self.MWR._grid.at_node['soil__thickness'][self.MWR._grid.at_node['mass__wasting_id'] < 1]*candidate_values[key]
                )
            else:
    ...
    def _determine_posterior_likelihood(self, prior_t):
        ...
        if self.calibration_method == "extent_only":
            self.omegaT = self._omegaT(metric = self.extent_metric)
            self.MSE_Qs = np.nan # neither of these metrics are run but need a value for MCMC_stats table
            self.SE_DoD = np.nan
            candidate_posterior = prior_t*self.omegaT
            ...
            self.MSE_Qs = self._MSE_Qs()
            
            # omegaT
            self.omegaT = self._omegaT(metric = self.extent_metric)
            
            # square error of the modeled DoD (SE_DoD)
            self.SE_DoD = self._SE_DoD(metric = self.extent_metric)
            
            # determine posterior likilhood: product of prior liklihood, omegaT, Qt 
            candidate_posterior = prior_t*self.omegaT*(1/self.MSE_Qs)*(1/self.SE_DoD)
    ...
    def _jump_or_stay(self, candidate_values, candidate_posterior, selected_values):
        ...
        # (new parameter has a probability of not being accepted equal to 1-acceptance ratio)
        # for first jump, the probability of not being accepted is 0
    ...
    def _MCMC_sampler(self, number_of_runs):
        ...
        for self.it in range(number_of_runs):
            ...
            # get candidate parameter values that will be used to run the model and become the new selected values based on model performance and chance
            for key in self.params:
                candidate_values[key], jump_sizes[key] = self._get_a_candidate_value(selected_values[key], key)
            ...
            self._simulation(candidate_values)
            
            # determine candidate parameter posterior likelihood value (posterior pdf value) 
            candidate_posterior = self._determine_posterior_likelihood(prior_t)
            ...
            # save statistics of parameters adjusted by the MCMC algorithm
            p_table = []
            p_nms = []
            for key in self.params:
            ...
            if self.it%self.N_cycles == 0:
                ...
                self._adjust_jump_variance(mean_acceptance_ratio)
                self.ar = [] # reset acceptance ratio tracking list
            
            # if print progress to screen
            if self.show_progress and self.it%5 == 0:        
...
def define_profile_nodes(mg):
    ...
    def quiet_ginput():
        # ginput throws a number of warnings and errors to the terminal screen
        # when the user clicks the mouse. The warnings do not prevent ginput
        # from running as expected but the messages do clutter the terminal 
        # screen. This function prevents those warnings and errors from 
        ...
        with open(os.devnull, 'w') as fnull:
            ...
            try:           
                ...
                # restore stderr to catch real errors outside of ginput
    ...
    def _coordinates_to_rmg_nodes(pts):
        ...
        while k < len(pts)-1:
            ...
            for i,v in enumerate(vals):
                ...
                # create list of all nodes that coincide with link
...
def plot_MCMC_jumps(results, parameter_1, parameter_2 = None, xlims = None, ylims = None):
    # plot jumps
    figsize = (6,3)
    fs = 12
    if parameter_1 is not None and parameter_2 is not None:
        fig, ax = plt.subplots(figsize = figsize)
        plt.plot(results['selected_value_{}'.format(parameter_1)], results['selected_value_{}'.format(parameter_2)], 
                  linestyle = '-', color = 'k',
                  alpha = 0.75, linewidth = 0.1,zorder=0)
        results_s = results.sort_values(by=['candidate_posterior'])
        z = results_s['candidate_posterior']
        sc = ax.scatter(results_s['selected_value_{}'.format(parameter_1)], results_s['selected_value_{}'.format(parameter_2)], c=z, s=5, cmap="plasma", alpha = 1,zorder=10)
        if xlims:
            plt.xlim(xlims)
        if ylims:         
            plt.ylim(ylims)
        plt.xlabel('{}'.format(parameter_1), fontsize = fs)
        plt.ylabel('{}'.format(parameter_2), fontsize = fs)
        plt.tick_params(axis = 'both', which = 'major', labelsize = fs)
        plt.colorbar(sc, label=r'$L(\Lambda)$')
    
    elif parameter_1 is not None and parameter_2 is None:
        
        fig, ax = plt.subplots(figsize = figsize)
        plt.plot(results['iteration'], results['selected_value_{}'.format(parameter_1)], 'k.')
        plt.xlabel('iteration', fontsize = fs)
        plt.ylabel('{}'.format(parameter_1), fontsize = fs)
        plt.tick_params(axis = 'both', which = 'major', labelsize = fs)
